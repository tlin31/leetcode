...
II。
	1. 定义 left[i]：arr[i] 作为最小值，被“左边”延伸多少次？

		找：左边最近的一个严格小于 arr[i] 的元素的距离

		即找：上一个更小元素（previous less element, PLE）

		当出现相等元素时，左边算严格大，以避免重复计算（去重）
		...
		遇到 arr[i] <= 栈顶 → 不断 pop，保持递增

		所以：left[i]=i−pleIndex

		若不存在更小元素，则 pleIndex = -1 → left[i] = i + 1

	2. 定义 right[i]：arr[i] 作为最小值，被“右边”延伸多少次？

		找：右边最近的一个“小于等于 arr[i]”的元素的距离

		右边采用：小于等于（≤），这样能正确处理重复值，确保每个重复值只被分配一次 

		right[i]=rleIndex−i

		若不存在更小或等值元素，则 rleIndex = n


	Note: 为什么左边用 <，右边用 <=？ 确保每个子数组的最小值由最左边的那个相同值承担，避免重复计数。

III。示例讲解	
	（arr = [3,1,2,4]）
		i	arr[i]	left（严格 >）	right（>=）
		0	3	1	1
		1	1	2	3
		2	2	1	2
		3	4	1	1
