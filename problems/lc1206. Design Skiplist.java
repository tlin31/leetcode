...
这个 Skiplist 用 next + down 两个指针来表示“横向链表 + 纵向索引”，
插入时用栈保存路径，自底向上抛硬币决定是否升层。

head 永远指向最高层，层数是动态增长的
...
class Skiplist {
    ...
    public void add(int num) {
        ...
        //Step 1：从顶层一路找插入位置，与search几乎一样，但是往下一层走之前，用stack保存走过的路径
        Node cur = head;
        while (cur != null) {
        ...
        // 如果升到最顶层还想升？
        // 新建一层, 新 head.down 指向旧 head, 新层暂时只有哨兵 📌 动态扩容层数
        if (insert) 
