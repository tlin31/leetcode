...
解题的关键在于：在每一步中，找到从当前 IP 开始、且包含不超过剩余数量 n 的最大对齐块。 

转换：将点分十进制 IP 转换为 32 位长整型。

找低位：利用 x & -x 找到当前 IP 的最低非零位（Lowbit），这决定了该 IP 作为 CIDR 起始地址时能支持的最大块大小。

确定大小：块大小 2^k 必须满足：2^k <= Lowbit && 2^k <=n
更新：记录当前块，将 IP 增加 2^k，并将 n 减少 2^k
...
public List<String> ipToCIDR(String ip, int n) {
    ...
    while (n > 0) {
        // 找到当前 IP 能够对齐的最大块大小
        long step = x & -x;
        if (x == 0) step = (long) Math.pow(2, 31); // 处理 0.0.0.0
        
        // 确保块大小不超过剩余数量 n
        while (step > n) step /= 2;
        
        res.add(longToIp(x, (int) (32 - Math.log(step) / Math.log(2))));
        x += step;
        n -= (int) step;
    }
    return res;
...
private String longToIp(long x, int prefix) {
    return String.format("%d.%d.%d.%d/%d", 
        (x >> 24) & 255, (x >> 16) & 255, (x >> 8) & 255, x & 255, prefix);
}
