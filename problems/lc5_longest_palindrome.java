...
例子：s = "aa", i = 0, j = 1。
j - i = 1（小于 3）。
如果不写 if < 3：代码会执行 dp[0][1] = dp[1][0]。
问题：dp[1][0] 在逻辑上是无效区间（左边界 > 右边界），虽然数组不会越界，但其初始值为 false。这会导致 dp[0][1] 错误地变为 false。

场景 B：子串长度为 3 (L = 3)
例子：s = "aba", i = 0, j = 2。
j - i = 2（小于 3）。
如果不写 if < 3：代码会执行 dp[0][2] = dp[1][1]。
虽然这里是对的（因为长度为 1 的已经在初始化时设为 true 了），但为了同时兼顾长度为 2 的情况，j - i < 3 是最简洁的统一处理方式。
