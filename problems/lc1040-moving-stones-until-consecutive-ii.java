...
key:
	- sliding windows
	- edge case:
		1) empty array, return [0,0]?
		2) 1 element, return [0,0]
...
method:
	...
	- Lower Bound - sliding windows
		...
		We want move other stones into this window.
		...
		window slide，不断构造一个区间，这个区间[i,j]满足，区间的长度不会大于石头的数量，即这个区间是可以被区间外部
		的石头塞满的(可以塞不下，但不能有空隙)。
		...
		如果不是以上的特殊情况，那么不需要检查区间内部是否连续了，也就是将除了[i,j]区间以外的数字放入[i,j]区间，
		放不下的要让它们连续，只需要n-(j-i+1)步。
	...
	- Upper Bound
		...
		如果第一次移动最右端，那么第一步一定是移到最右边开始第二个的前面，即stones[n-2]，n为stones的长度，
		并且最左的位置是stones[0]，因此移动距离就是stones[n-2]-stones[0]-1-(n-3)，也就是当前区间内部的位置，
		减去内部已经有石头的数量(有3个不在区间内部，分别是区间左右端2个和最后1个stones[n-1])，
		剩下的空位置就是能移动石头的位置。

		例如：[1,3,5,7,12,65]，1到12之间还剩下7个空位置，也就是先移动右端，能最多移动7次。

		同样的，如果第一次移动最左端，那么第一步一定是移动到最左边开始第二个的后面，即stones[1]，它的最右端即是
		stones[n-1]，因此移动距离就是stones[n-1]-stones[1]-1-(n-3)；
...
stats:
	- Time of quick sorting O(NlogN), Time of sliding window O(N)
	- Space O(1)
 ...
 public int[] numMovesStonesII(int[] A) {
        ...
        // n is the length of the array, 2 is the endpoints
